...
## Summary
Collection Lists are a returning feature from the original [YTV Sitekick](/docs/ytv/ytv_sitekick/), with some small changes.  Collection lists are released daily, weekly, and monthly, as well as a few themed collections which have no expiry date.  Each chip collected grants 1XP to the player.
...
[YTV Sitekick](/docs/ytv/ytv_sitekick/) featured two types of collection lists: monthly and sponsored.  Monthly collection lists could be accessed for three months before they would become inaccessible.  It is assumed that sponsored lists would appear until a date negotiated by YTV/Corus and the advertiser.
